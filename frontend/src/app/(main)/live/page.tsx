...
import { Play, Search, ChevronDown, SlidersHorizontal, Plus, X } from "lucide-react"
        ...
        <>
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-zinc-950 border-r border-white/10 z-50 p-6 flex flex-col md:hidden animate-in slide-in-from-left duration-300">
            ...
            <div className="flex-1 overflow-y-auto space-y-1">
              ...
              >
                <span>All Channels</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-white/10">{totalChannelsCount}</span>
              </button>
              {filteredCategories.map(cat => (
                <button
                  key={cat.id}
                ...
                >
                  <span className="truncate pr-2">{cat.name}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full flex-shrink-0 bg-white/10">{cat._count?.channels || 0}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div ref={gridContainerRef} className="flex-1 overflow-y-auto p-4 sm:p-8 relative">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div className="flex items-center justify-between w-full md:w-auto">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-white font-outfit">{activeCategory?.name || "All Channels"}</h1>
