...
export class ChannelWorker {
  ...
  start(): void {
    ...
    this.active = true;
    this.logger.log(`[Worker:${this.channelId}] Starting worker for stream: ${this.streamUrl}`);
    ...
    this.pmtParser.onTracksFound((tracks) => {
      ...
      this.logger.log(`[Worker:${this.channelId}] Detected audio tracks: ${JSON.stringify(tracks)}`);
    });
    ...
    this.continuity.on('error', (err) => {
      this.logger.error(`[Worker:${this.channelId}] Stream continuity error: ${err.message}`);
    });
  ...
  private spawnFfmpeg(): void {
    ...
    this.logger.log(`[Worker:${this.channelId}] Spawning FFmpeg ingest process. Reconnect attempt: ${this.reconnectCount}`);
    
    // Reset session inside the continuity engine so it calculates the next offset
    ...
    const args: string[] = [
      ...
      '-c:v', 'copy',
      '-c:a', 'copy', // zero transcode during ingest
    ...
    try {
      ...
      // Pipe FFmpeg stdout into our aligner
      ...
      this.ffmpegProcess.stderr?.on('data', (data: Buffer) => {
        ...
        if (msg.includes('Error') || msg.includes('error') || msg.includes('warning') || msg.includes('Warning')) {
          this.logger.warn(`[FFmpegIngest:${this.channelId}] ${msg}`);
        }
      });

      this.ffmpegProcess.on('close', (code) => {
        this.logger.log(`[Worker:${this.channelId}] FFmpeg closed with code ${code}`);
      ...
      this.logger.error(`[Worker:${this.channelId}] Failed to spawn FFmpeg: ${err.message}`);
      this.handleReconnect();
    }
  }
  ...
  private handleReconnect(): void {
    ...
    this.reconnectCount++;
    const delay = Math.min(1000 * Math.pow(2, Math.min(this.reconnectCount - 1, 4)), 10000); // Exponential backoff up to 10s
    this.logger.log(`[Worker:${this.channelId}] Scheduling FFmpeg ingest reconnect in ${delay}ms`);
  ...
  /**
   * Increment subscribers, cancel any idle destroy timer.
   */
  incrementSubscribers(): void {
    ...
    this.reconnectCount = 0; // Reset reconnect backoff on active viewer attach
  }

  /**
   * Decrement subscribers, start idle destroy timer if active count drops to 0.
   */
  decrementSubscribers(onIdleDestroy: () => void): void {
    if (this.ringBuffer.subscriberCount === 0) {
      this.logger.log(`[Worker:${this.channelId}] 0 active viewers left. Scheduling shutdown in 10s.`);
      if (this.idleTimeout) clearTimeout(this.idleTimeout);
      this.idleTimeout = setTimeout(() => {
        this.logger.log(`[Worker:${this.channelId}] Idle timeout reached. Destroying worker.`);
        this.destroy();
        onIdleDestroy();
  ...
  /**
   * Destroy worker, terminate FFmpeg process, clear memory buffers.
   */
  destroy(): void {
    if (this.destroyed) return;
    this.destroyed = true;
    this.active = false;
    this.logger.log(`[Worker:${this.channelId}] Destroying worker.`);
